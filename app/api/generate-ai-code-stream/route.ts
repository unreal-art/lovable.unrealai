  ...
  try {
    ...
    ;(async () => {
      try {
            ...
            try {
              ...
              if (intentResponse.ok) {
                ...
                const searchExecution = executeSearchPlan(
                  searchPlan,
                  Object.fromEntries(
                    Object.entries(global.sandboxState.fileCache.files).map(
                      ([path, data]) => [
                ...
                if (
                  ...
                  if (target) {
                    await sendProgress({
                      type: "status",
                      message: `✅ Found target: ${target.filePath
                        .split("/")
                        .pop()}`,
                    })
                    enhancedSystemPrompt = `${formatSearchResultsForAI(
                      searchExecution.results
              ...
              console.error(
                "[generate-ai-code-stream] Error in agentic search:",
                error
              )
        ...
        const conversationContext = buildConversationHistoryPrompt(
          global.conversationState
        )
...
${enhancedSystemPrompt}`
        ...
        if (Object.keys(backendFiles).length > 0) {
          if (editContext?.primaryFiles.length > 0) {
            const primaryFiles = await getFileContents(
              editContext.primaryFiles,
              global.sandboxState.fileCache.manifest!
            )
            const contextFiles = await getFileContents(
            ...
            const allFilesContent = Object.entries(backendFiles)
              .map(
                ([path, data]) =>
                  `<file path="${path}">\n${data.content}\n</file>`
              )
        ...
        const files = (
          generatedCode.match(/<file path="[^"]+">[\s\S]*?<\/file>/g) || []
        ).map((fileBlock) => {
